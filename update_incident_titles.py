#!/usr/bin/env python3
"""Fix titles for Incident documents in the local EPA Ireland SQLite DB.
...
Usage:
    python3 update_incident_titles.py [--db epa_ireland.db]

The script is idempotent and safe to run multiple times. You may wish to run
`VACUUM` afterwards to compact the DB.
"""
...
def main(db_path: Path) -> None:
    ...
    rows = cur.fetchall()
    print(f"Found {len(rows)} incident documents with blank titles.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Populate title from metadata subject for Incident documents.")
